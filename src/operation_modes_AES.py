...
def aes_encrypt(message, key, mode):
    ...
    if mode == 'ECB':
        ...
        cipher_text = cipher.encrypt(padding(message)) # cifra a mensagem com padding
        return cipher_text, base64.b64encode(cipher_text).decode() # retorna o texto cifrado e o texto cifrado em base64
        ...
        return cipher_text, base64.b64encode(init_vector + cipher_text).decode()
        ...
        cipher_text = cipher.encrypt(message)
        return cipher_text, base64.b64encode(nonce + cipher_text).decode()




key = b'0123456789abcdef'  # Chave de 16 bytes (128 bits)
message = b"oooooooooooooooooooooooooooooooo"
...
for mode in modes:
    ...
    start = time.time()
    cipher_text, b64 = aes_encrypt(message, key, mode)
    print(f"Modo de operação: {mode}")

    print(f"Tempo de execução: {(time.time() - start):.6f} segundos\n")
